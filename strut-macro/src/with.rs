...
use crate::{
    util::field::{chk_named_st, chk_st, get_field_trim, get_fields},
    WITH,
};

pub(crate) fn with_field(input: TokenStream) -> TokenStream {
    // 解析结构体的抽象语法树
    ...
    // 取所有有效成员变量
    let filter_fields = get_fields(fields, WITH);
    ...
    // 为成员生成 with 函数
    let with_fns = filter_fields.iter().map(|f| {
        let f_name = f.ident.to_owned().unwrap(); // 成员名字
        let f_ty = f.ty.to_owned(); // 成员类型
        let f_fns_name = format_ident!("with_{}", f_name); // 成员 set 函数名
